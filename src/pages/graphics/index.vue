...
import { IMAGEURL, RequestApi } from '@/utils/request'
import { Graphics } from '@/@types/frontPage'
import { myData } from '@/store/useMyData'

// 上传图片
async function upload() {
  if(submitData.ins_report.length >= 3) {
    uni.showToast({ title: '最多上传三张图片', icon: 'none', duration: 1000 })
    return
  }
...
async function subMit() {
  ...
  const res = (await RequestApi.GrapHics(submitData)) as {
    statusCode: number
  }
