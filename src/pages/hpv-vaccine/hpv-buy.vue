...
import { Hpvcomboname, Hpvcombotime } from '@/@types/frontPage'
...
onLoad(async (event: any) => {
  ...
  hpv_combo_name.value = [res.data[0]]
  hpv_combo_time.value = [res.data[1]]
})
...
async function hpvPrice() {
  ...
  hpv_combo_price.value = res.data[0].price
  uni.hideLoading()
}
