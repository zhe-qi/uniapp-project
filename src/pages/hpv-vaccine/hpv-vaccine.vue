...
import { Hpvselect, Hpvlist } from '@/@types/frontPage'
...
onMounted(async () => {
  const res = (await RequestApi.OtuHpv()) as Resdata
  hpv_select.value = res.data[0].hpv_select
  hpv_list.value = res.data[0].hpv_list
  hpv_all.value = res.data[0].hpv_list
...
<template>
  ...
  <view class="tab-view">
    ...
    >
      <text>{{ item.name }}</text>
      <text :class="index == checked ? 'checked' : ''" />
    </view>
  </view>
  <!-- 疫苗列表 -->
  <view
  ...
  >
    ...
    <view
      class="hpv-reserve"
      @click="toReserve(item._id, item.name, item.price, item.describe)"
    >
      去预约
    </view>
  </view>
