...
import { Phyterm, Phydata } from '@/@types/frontPage'
...
onMounted(async () => {
  ...
  phy_term.value = res[0].data
  phy_data.value = res[1].data
...
async function queryResult() {
  const res = (await RequestApi.PhyQuery(filter_data)) as { data: [] }
  phy_data.value = res.data
}
