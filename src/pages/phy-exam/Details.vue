...
import { Phydetail } from '@/@types/frontPage'
// 引入弹窗组件
import SeMember, { Event } from '@/components/SeMember.vue'
...
onLoad(async (event: any) => {
  ...
  const res = (await RequestApi.PhydaTeil({ id: eventData.id })) as { data: Phydetail[] }
  S_Data.phy_name = eventData.title
  phy_detail.value = res.data
  // 骨架屏
...
// 存储传递给子组件的方法
let S_Data = reactive<Event>({
  phy_name: '',
  phy_time: '',
  show: true
})
// 调用子组件的方法:选择成员打开弹窗
let compNone = ref<typeof SeMember | null>(null)
function memBer() {
...
<template>
  <view
    v-for="(item, index) in phy_detail"
    :key="index"
    class="details-Top"
  >
    <image
  ...
  <view class="nucleic-time">
    ...
    <view class="nucleic-crowd">
      ...
      >
        <view
          v-for="(item_a, index_a) in item.crowd"
          :key="index_a"
        >
          <image
            :src="item_a.image"
  ...
  <view class="nucleic-time">
    ...
    >
      <block
        v-for="(item_a, index_a) in item.project"
        :key="index_a"
      >
        <text class="project-Title">{{ item_a.title }}</text>
        <view
          v-for="(item_b, index_b) in item_a.content"
          :key="index_b"
          class="project-card"
        >
  ...
  <view class="Total-view">
    <text v-if="phy_detail.length > 0">
      检测费用：¥{{ phy_detail[0].price }}
    </text>
    <text @click="memBer">选择成员</text>
  </view>
  <!-- 弹窗选择成员 -->
  <SeMember ref="compNone" />
  <!-- 骨架屏 -->
