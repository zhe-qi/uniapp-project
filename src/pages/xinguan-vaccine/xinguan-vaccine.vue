...
import { Newapptime } from '@/@types/frontPage'
...
async function Submit() {
  ...
  if (res.msg == '提交成功') {
    uni.hideLoading()
    uni.navigateTo({ url: '/pages/my-service/xinguan/xinguan' })
  } else {
    uni.hideLoading()
...
<template>
  ...
  <view>
    ...
    <view class="reserve-address">
      <text class="reserve-name">
        {{ timeData.data.Hospital }}
      </text>
      <text class="reserve-road">
        {{ timeData.data.address }}
      </text>
      <view class="reserve-title">
        <text
          v-for="(item, index) in timeData.data.company"
          :key="index"
        >
          {{ item }}
        </text>
