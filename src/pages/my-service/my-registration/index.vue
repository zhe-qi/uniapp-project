...
import { Registratorder } from '@/@types/frontPage'

// 获取订单数据
let order = ref<Registratorder[]>([])
let show = ref(false)
onShow(async () => {
  const res: any = (await RequestApi.UserreGistrat()) as{ data: any[] }
...
async function Cancel(id: string, index: number) {
  const res: any = (await RequestApi.RegistCancel({ _id: id })) as {
    statusCode: number
  }
