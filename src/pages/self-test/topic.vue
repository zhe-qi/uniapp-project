...
import {Topic} from '@/@types/frontPage'
...
let res = ref<{data:Topic[]} | null>(null)
onShow(async()=>{
  // 清空上一次的结果
  add_to.value = 1
  add_to_s.value = 0
  percent.value = 0
  topic_id.value = []
  if(judg_valu.value == '001'){//抑郁症测评；
    res.value = await RequestApi.DepressionTopics() as {data:Topic[]}
  }else if(judg_valu.value == '002'){//男性功能测评；
    res.value = await RequestApi.PrematureTopics() as {data:Topic[]}
  }else{//失眠程度测评
    res.value = await RequestApi.InsomniaTopics() as {data:Topic[]}
    // console.log(res)
  }
  topic_data.value = res.value.data
  topic_length.value = res.value.data.length
  each_question.data = res.value.data[0]
...
watch([add_to_s,topic_length],(newVal)=>{
