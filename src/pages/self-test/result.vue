...
import { Testres, Sharedata } from '@/@types/frontPage'
...
let res = ref< { data: Testres[] } | null>(null)
onShow(async () => {
  if (type_id.value == '001') {
    //抑郁症
    res.value = (await RequestApi.DepressiSon({ value: topic_id_v.value })) as  { data: Testres[] }
  } else if (type_id.value == '002') {
    //男性功能
    res.value = (await RequestApi.PremaTure({ value: topic_id_v.value })) as  { data: Testres[] }
  } else {
    //失眠测评
    res.value = (await RequestApi.InsoMnia({ value: topic_id_v.value })) as  { data: Testres[] }
  }
  test_res.value = res.value.data
...
<template>
  <view class="back-color">
    ...
    >
      ...
      <view
        v-for="(item_a, index_a) in item.outline"
        :key="index_a"
      >
        {{ index_a + 1 }}. {{ item_a }}
      </view>
