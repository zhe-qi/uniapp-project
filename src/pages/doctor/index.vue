<template>
  <view class="doctor-top filter-view">
    ...
    >
      ...
      >
        <text>{{ item.date }}</text>
        <text :class="index == selectIndex ? 'changeColor' : ''">
          {{
            item.week
          }}
        </text>
  ...
  <!-- 提示组件 -->
  <Point
    :show="doctor_list.length == 0 ? true : false"
    :title="title"
  />
  <!-- 骨架屏 -->
...
import { Doctortime, Doctorlist } from '@/@types/frontPage'
...
onLoad(async (event: any) => {
  ...
  doctor_time.value = res[0].data
  doctor_list.value = res[1].data
...
async function allDoctor() {
  selectIndex.value = -1
  const res = (await RequestApi.AlldList({ dep_id: dep_id.value })) as { data: any[] }
  doctor_list.value = res.data
}
// 根据日期筛选医生列表
async function selectDoctor(index: number, dep_id: string, week: string) {
  selectIndex.value = index
  const res = (await RequestApi.EverydList({ dep_id, week })) as { data: any[] }
  doctor_list.value = res.data
}
// 跳转医生主页
