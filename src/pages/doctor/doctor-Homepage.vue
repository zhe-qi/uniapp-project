<template>
  <block
    v-for="(item, index) in doctor_data"
    :key="index"
  >
    <!-- 个人信息 -->
  ...
  <view class="prompt">
    <text>重要提示:</text>
    <text>
      1.本次预约就诊当天不可取消预约,如需取消,请在就诊前一天的24:00之前操作,累计取消或爽约三次可能会被列入医院黑名单,请按需预约
    </text>
    <text>
      2.预约挂号暂不支持医保支付,若本次挂号使用手机在线支付,就诊过程中的门诊检验检查,处方费用可能不支持医保支付
    </text>
  </view>
  <!-- 弹窗组件 -->
  <popups ref="compNone" />
...
import { Doctorhome } from '@/@types/frontPage'
// 预约弹窗组件
import popups, { Data } from '@/components/PopUps.vue'
...
onLoad(async (event) => {
  ...
  _id.value = id
  const res = (await RequestApi.DoctorHome({ _id: id })) as { data: Doctorhome[] }
